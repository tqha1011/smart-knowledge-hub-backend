...
export class ChatMessage {
  ...
  /**
   * The length cap only applies to `User` messages — it exists to bound what a
   * client can submit, matching ChatMessageRequestDto's `@MaxLength(4000)`. An
   * `Assistant` message is generated by the RAG pipeline, not typed by a user, and
   * a thorough answer can legitimately run longer than that.
   */
  private static validateInformation(
    params: ChatMessageCreateParams,
  ): Result<void, ChatMessageDomainErrorValidation> {
    if (params.role === CommonChatRole.User && params.content.length > 4000) {
      return err(
